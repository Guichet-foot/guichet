...
import { openOdcavScanSession, closeOdcavScanSession } from "@/lib/actions/billeterie-session-actions";
...
export function OdcavSessionButton({ openUntil: initialOpenUntil }: { openUntil: string | null }) {
  ...
  async function handleOpen() {
    setLoading(true);
    const result = await openOdcavScanSession();
    setLoading(false);
    if (result.error) { toast.error(result.error); return; }
    setOpenUntil(result.openUntil!);
    setRemaining(getRemaining(result.openUntil!));
    toast.success("Billeterie ODCAV ouverte pour 10h");
  }

  async function handleClose() {
    setLoading(true);
    const result = await closeOdcavScanSession();
    setLoading(false);
    if (result.error) { toast.error(result.error); return; }
    setOpenUntil(null);
    setRemaining(0);
    toast.success("Billeterie ODCAV fermée");
  }
