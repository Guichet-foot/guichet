...
async function toJpegDataUrl(
  ...
  if (sharpMod) {
    try {
      ...
      // tag and physically bake the rotation in — sanitizing first would strip that tag and
      // silently produce a wrongly-rotated photo. Sharp's own re-encoded output is already
      // clean (no ICC/DQT quirks), so no need to sanitize afterwards either.
