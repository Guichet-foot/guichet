...
const INFO_W   = CARD_W * 0.65;
const QR_COL_W = CARD_W * 0.35;
const PHOTO_D  = CARD_H * 0.40;    // ~21.6mm
...
// ── SVG Icons (Lucide paths, white stroke on green box) ────────────
const ICON_SIZE = 5; // pt inside the 7pt green box
...
// ── Icon box: green square + centered SVG icon ─────────────────────
const ICON_BOX = 7.5; // pt
  ...
  return (
    <View style={{
      ...
      <View style={{
        ...
        {/* Logo — transparent, no white wrapper */}
        <Image
          src={card.logoDataUrl}
          style={{ width: HEADER_H - 4, height: HEADER_H - 4, objectFit: "contain" }}
        />

        {/* Title — centered, right-padding leaves room for photo */}
        <View style={{
          flex: 1, alignItems: "center",
          paddingRight: PHOTO_D * 0.85,
        }}>
          <Text style={{ fontFamily: "Helvetica-Bold", fontSize: 8.5, color: "#1a5c2a", lineHeight: 1.05 }}>
            CARTE D&apos;ACCÈS
          </Text>
          <Text style={{ fontFamily: "Helvetica", fontSize: 4, color: "#166534", marginTop: 1.5 }}>
            — SAISON {saison} —
          </Text>
          <View style={{
            marginTop: 2.5, backgroundColor: typeColor, borderRadius: 99,
            paddingHorizontal: 5, paddingVertical: 1.5,
          }}>
            <Text style={{ fontFamily: "Helvetica-Bold", fontSize: 3, color: "white", letterSpacing: 0.6 }}>
              {typeLabel}
            </Text>
          </View>
        </View>
      ...
      <View style={{
        ...
        width: CARD_W, height: BODY_H, flexDirection: "row",
      }}>
        {/* Info column — 65% */}
        <View style={{
          ...
          {rows.map(({ label, value, icon }, i) => (
            <View key={label} style={{
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={{
                  fontFamily: "Helvetica-Bold", fontSize: 3,
                  color: "#166534", letterSpacing: 0.3,
                }}>
                  {label}
                </Text>
                <Text style={{
                  fontFamily: "Helvetica-Bold", fontSize: 5,
                  color: "#111", marginTop: 0.8,
                }}>
                  {value}
                </Text>
              </View>
        ...
        <View style={{
          ...
          <View style={{ borderWidth: 0.5, borderColor: "#1a5c2a", padding: 1 }}>
            <Image
              src={card.qrDataUrl}
              style={{ width: QR_COL_W * 0.68, height: QR_COL_W * 0.68 }}
            />
          </View>
        </View>
      </View>
...
const A4_W = 595.28;
const A4_H = 841.89;
const GAP_H     = 7 * MM;                              // 19.84 pt between cols
const MARGIN_H  = (A4_W - 2 * CARD_W - GAP_H) / 2;   // ≈ 15.1 mm each side
const MARGIN_V  = 10 * MM;                             // 28.35 pt top/bottom
const GAP_V     = 5 * MM;                              // 14.17 pt between rows
const STRIDE_V  = CARD_H + GAP_V;
const ROWS_PER_PAGE = Math.floor((A4_H - 2 * MARGIN_V + GAP_V) / STRIDE_V); // 4
